...
class MiltonFilter extends Filter {

  def doFilter(p1: ServletRequest, p2: ServletResponse, p3: FilterChain) = {
    MiltonInitializer.webdavManager.process(new WDRequest(p1.asInstanceOf[HttpServletRequest]),
      new WDResponse(p2.asInstanceOf[HttpServletResponse]))
  }
